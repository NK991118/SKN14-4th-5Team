...
def select_question(request):
    """사용자가 PDF 문제지를 선택하는 페이지를 보여주는 뷰입니다."""
    # config.py의 대학 데이터를 템플릿으로 전달하여 자바스크립트가 사용할 수 있게 합니다.
    context = {'schools_data_json': UNIVERSITY_DATA}
    return render(request, 'app/01_select_question.html', context)


# [3] 나의 답안 입력 페이지 뷰 (드디어 완성!)
@login_required(login_url='app:login')
def my_answer(request):
    """사용자가 답안 이미지를 업로드하고 문제 ID를 선택하는 페이지의 뷰입니다."""
    
    # 사용자가 폼을 통해 데이터를 '제출'했을 때 (POST 방식)
    if request.method == 'POST':
        # 1. 폼에서 'question_id'와 'answer_image' 데이터를 추출합니다.
        #    <input> 태그의 name 속성이 각각 'question_id', 'answer_image'인 값을 찾아.
        question_id = request.POST.get('question_id')
        # 파일 데이터는 request.POST가 아니라 request.FILES에 따로 담겨서 와!
        answer_image = request.FILES.get('answer_image')

        # 2. 이미지가 제대로 업로드되었는지 확인하고, 있다면 OCR을 실행해.
        extracted_text = "업로드된 이미지가 없습니다."
        if answer_image:
            # 우리가 ocr.py에 만들어둔 함수를 호출해서 이미지 파일에서 텍스트를 추출!
            extracted_text = process_image_to_text(answer_image)

        # 3. 처리된 데이터를 다음 페이지로 넘겨주기 위해 '세션(session)'에 저장해.
        #    세션은 사용자의 개인 사물함 같은 거야. 브라우저를 닫기 전까지 데이터가 유지돼.
        request.session['question_id'] = question_id
        request.session['extracted_text'] = extracted_text
        
        # 4. 모든 처리가 끝나면 'AI 첨삭 결과' 페이지로 이동(redirect)시켜.
        #    reverse('app:03_ai_result')는 /ai-result/ 라는 URL 주소를 만들어주는 함수야.
        return redirect(reverse('app:run_grading'))

    # 사용자가 페이지에 처음 '접속'했을 때 (GET 방식)
    else:
